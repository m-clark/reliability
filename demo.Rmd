...
Cronbach's $\alpha$ is a measurement of reliability that is a function of the average correlation among the items as well as the number of items.  While it is descriptive in nature, it assumes a unidimensional factor structure, or in other words, that all the items correspond to the same underlying construct[^chron_app].  The formula only requires the two values of the number of items $k$ and average inter-item correlation $\bar{r}$.

$$\alpha = \frac{k\bar{r}}{1+(k-1)\bar{r}}$$

All else being equal, simply increasing the number of observations/items will give you a higher reliability.  In some contexts this may make sense, but in others it does not.
