...
For simplicity we will focus on the person variance only in our examples. To calculate it we will use lme4 to run the mixed model using the long form data.

```{r mixed_model}
library(lme4)
model_gtheory = lmer(score ~ item + (1|person), neuroticism_long)
summary(model_gtheory, cor=F)
VarCorr(model_gtheory)
```

```{r}
psych::ICC(neuroticism)
