...
In our example, each person sees the five items, so those scores are not independent. In other words, the multiple observations are clustered within individuals.  This becomes more clear when we consider our data in 'long' format, as follows.


```{r neuro_long_form}
neuroticism_long = neuroticism %>% 
  rowid_to_column(var='person') %>% 
  gather(key = item, value=score, -person)
...
In this case we can treat the person or item as a source of variance, or even their interaction. For simplicity we will focus on the person variance only.
